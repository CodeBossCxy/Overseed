...
export default function InfluencerProfile({ influencer }: InfluencerProfileProps) {
  ...
  const zh = locale === 'zh'
  const pv = t.influencer.publicView
  ...
  const stats = [
    ...
    {
      label: pv.verified,
      value: influencer.isVerified ? pv.yes : pv.no,
    ...
    <div className="max-w-4xl mx-auto">
        ...
        <div className="px-6 pb-6">
          ...
          <div className="flex items-end gap-5 -mt-14 mb-4">
            ...
            <div className="min-w-0 -mb-2">
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-600 mt-1">
                ...
                <span>
                  {pv.memberSince}{formatMonthYear(influencer.user.createdAt, locale)}
                </span>
              </div>
            </div>
          </div>
          ...
          {influencer.languages.length > 0 && (
            <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
              <span className="font-medium text-gray-900">{pv.languages}</span>
          ...
          {influencer.bio && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{pv.about}</h3>
      ...
      <div className="mt-6 bg-white rounded-2xl shadow-md p-6">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          <h3 className="text-lg font-semibold text-gray-900">{pv.connectedPlatforms}</h3>
          {lastUpdated && (
        ...
        {accounts.length === 0 ? (
          <p className="py-8 text-center text-sm text-gray-500">{pv.noAccountsLinked}</p>
        ) : (
          <>
              ...
              {visibleAccounts.map((acc) => (
                <div key={acc.id} className="flex items-center gap-4 px-4 py-3.5">
                  ...
                  <div className="w-24 flex-shrink-0 hidden sm:block">
                    ...
                    <p className="text-xs text-gray-500">{pv.engagement}</p>
                  </div>
            ...
            {accounts.length > 3 && (
              ...
              >
                {showAll ? pv.showLess : pv.viewAllPlatforms}
